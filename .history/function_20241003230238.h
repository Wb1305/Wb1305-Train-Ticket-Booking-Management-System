...
void DocDuLieu_VeTau(VeTau arr[M], int &size)
{
    ...
    if (!f.is_open())
    {
        ...
        return; // Thoát hàm nếu không mở được file
    }

    char str[255];
    while (f.getline(str, 255)) // Đọc dòng vào str
    {
        string id = str;
        arr[size].setID_VT(id);

        if (!f.getline(str, 255))
            break; // Đọc tiếp, nếu không có dòng thì thoát
        arr[size].setLoaiVe(atoi(str));

        if (!f.getline(str, 255))
            break; // Đọc tiếp, nếu không có dòng thì thoát
        arr[size].setPrice(atoi(str));

        if (!f.getline(str, 255))
            break; // Đọc tiếp, nếu không có dòng thì thoát
        arr[size].setSoGhe(atoi(str));

        if (!f.getline(str, 255))
            break; // Đọc tiếp, nếu không có dòng thì thoát
        arr[size].setSoToa(atoi(str));

        if (!f.getline(str, 255))
            break; // Đọc tiếp, nếu không có dòng thì thoát
        arr[size].setChuyen(atoi(str));
...
double DoanhThuVeThuong(VeTau arr[M], int size)
{

    double sum = 0;
    for (int i = 0; i < size; i++)
        if (arr[i].getLoaiVe() == 2)
            sum += arr[i].getPrice();

    return sum;
}

double DoanhThuVeVip(VeTau arr[M], int size)
{

    double sum = 0;
    for (int i = 0; i < size; i++)
        if (arr[i].getLoaiVe() == 1)
            sum += arr[i].getPrice();
