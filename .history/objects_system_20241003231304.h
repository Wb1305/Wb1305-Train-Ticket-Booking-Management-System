...
{
private:
    // Có thể sử dụng this để lấy giá trị từ đối tượng hiện tại
    string id;
    string name;
    string phone_number;
    string cmnd;
...
public:
    PhieuVeTau() : VeTau() // Khởi tạo lớp cha
    {
        this->id = getID(); // Gọi phương thức từ lớp hiện tại
        this->name = getName();
        this->phone_number = getPhoneNumber();
        this->cmnd = getCMND();
        this->price = getPrice();
        this->loaiVe = getLoaiVe();
        this->soToa = getSoToa();
        this->soGhe = getSoGhe();
    }
    ...
    void showInfo()
    {
        HanhKhach::showInfo(); // Gọi thông tin HanhKhach
        VeTau::showInfo();     // Gọi thông tin VeTau
    }
};
